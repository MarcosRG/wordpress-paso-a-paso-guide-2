...
// Hook para processar dados durante o checkout (antes de criar a ordem)
add_action('woocommerce_checkout_process', 'bikesul_procesar_checkout_seguro', 10);

function bikesul_procesar_checkout_seguro() {
    // Validar se há produtos de seguro no carrinho
    $cart = WC()->cart;
    if (!$cart) {
        return;
    }
    ...
    foreach ($cart->get_cart() as $cart_item_key => $cart_item) {
        ...
        if ($is_insurance === 'yes' || $has_insurance_data) {
            ...
            // Validar dados essenciais do seguro
            $price_per_bike = isset($cart_item['insurance_price_per_bike_per_day']) ?
                floatval($cart_item['insurance_price_per_bike_per_day']) : 0;
            $total_bikes = isset($cart_item['insurance_total_bikes']) ?
                intval($cart_item['insurance_total_bikes']) : 0;
            $total_days = isset($cart_item['insurance_total_days']) ?
                intval($cart_item['insurance_total_days']) : 0;

            // Validações
            if ($total_bikes <= 0) {
                $insurance_errors[] = 'Número de bicicletas inválido para o seguro.';
            }
            if ($total_days <= 0) {
                $insurance_errors[] = 'Número de dias inválido para o seguro.';
            }
            if ($price_per_bike < 0) {
                $insurance_errors[] = 'Preço do seguro inválido.';
            }

            error_log("BIKESUL CHECKOUT PROCESS: Seguro validado - Preço: €{$price_per_bike}, Bicis: {$total_bikes}, Dias: {$total_days}");
        }
    }

    // Se há erros, mostrar e parar o checkout
    if (!empty($insurance_errors)) {
        foreach ($insurance_errors as $error) {
            wc_add_notice($error, 'error');
        }
        return;
    }

    // Log de sucesso se há seguro
    if ($has_insurance) {
        error_log("BIKESUL CHECKOUT PROCESS: Validação de seguro concluída com sucesso");
    }
}

// Hook para criar items na ordem
